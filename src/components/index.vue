<template>
  <div>
      ...
      <el-row class="hidden-xs-only">
        ...
        <el-col :span="9" :offset="3">
          <div class="wapper-panl">
            ...
            <div>
              <el-button @click="qiandao" class="panl-button" type="danger" :disabled="this.$store.state.disabledQ">签到
              </el-button>
            </div>
            <div>
              <el-button class="panl-button" @click="dialogVisible1 = true" type="danger"
                :disabled="this.$store.state.disabledD">听歌打卡</el-button>
            </div>
            <div>
              <el-button @click="centerDialogVisible1 = true" v-show="loginButtonShow" class="panl-button" type="danger"
                :disabled="this.$store.state.disabledIn">登陆</el-button>
            </div>
            <div>
              <el-button @click="LoginOut" v-show="loginOutShow" class="panl-button" type="danger"
                :disabled="this.$store.state.disabledOut">注销登陆</el-button>
            </div>
            <br />
            <el-divider></el-divider>
            <center>
              <span>百日笛子千日萧，一把二胡拉折腰！</span>
            </center>
          </div>
        </el-col>
      ...
      <el-row type="flex" justify="center">
        <el-col class="hidden-sm-and-up" :span="18">
          <div class="wapper-panl">
            ...
            <div>
              <el-button @click="centerDialogVisible = true" v-show="loginButtonShow" class="panl-button" type="danger">
                登陆</el-button>
            </div>
            <div>
            ...
            <center>
              <p>百日笛子千日萧，一把二胡拉折腰</p>
              <el-button type="text" @click="dialogVisible3 = true">关于Clockin</el-button>
            </center>
          </div>
        </el-col>
      </el-row>
    ...
    <!-- PC端选择打卡界面 -->
    <el-dialog title="选择播放内容" :visible.sync="dialogVisible1" width="30%" :before-close="handleClose">
      <el-checkbox-group v-model="checkList">
        ...
        <el-tooltip class="item" effect="dark" content="为了不影响您的听歌口味，此项必选，推荐歌曲备选" placement="top-start">
          <el-checkbox label="我喜欢的音乐" disabled></el-checkbox>
        </el-tooltip>
      </el-checkbox-group>
      <span slot="footer" class="dialog-footer">
    ...
    <!-- 移动端打卡选择界面 -->
    <el-dialog title="选择播放内容" :visible.sync="dialogVisible2" width="70%" :before-close="handleClose">
      <el-checkbox-group v-model="checkList">
        <el-checkbox label="日推歌曲"></el-checkbox>
    ...
    <!-- 移动端登陆弹窗 -->
    <el-dialog title="登陆网易云音乐" :visible.sync="centerDialogVisible" width="90%" center>
      <el-input placeholder="手机号码/绑定邮箱" prefix-icon="el-icon-user-solid" v-model="cellphone"></el-input>
      <el-input show-password style="margin-top : 20px" placeholder="请输入密码" prefix-icon="el-icon-s-help"
    ...
    <!-- PC界面登陆弹窗 -->
    <el-dialog title="登陆网易云音乐" :visible.sync="centerDialogVisible1" width="30%" center>
      <el-input placeholder="手机号码/绑定邮箱" prefix-icon="el-icon-user-solid" v-model="cellphone"></el-input>
      <el-input show-password style="margin-top : 20px" placeholder="请输入密码" prefix-icon="el-icon-s-help"
        v-model="password"></el-input>
      <div style="margin-top: 20px">
    ...
    <!-- 开屏弹窗 -->
    <!-- 现在使用本地服务器所以注释了 -->
    <!-- <el-dialog title="请注意" :visible.sync="tost" width="50%" :before-close="handleClose">
      &nbsp;&nbsp;&nbsp;&nbsp;由于网易服务器高频ip限制，同时也为了缓解我服务器的压力，于是我设置了缓存，缓存时间为2分钟，相同的请求会在2分钟内只向网易服务器发一次请求，
      <br />&nbsp;&nbsp;&nbsp;&nbsp;所以这里建议您操作一个账号之后等待3-4分钟再操作另外的账号，否则你全都是在为我在刷
      = =
      <br />&nbsp;&nbsp;&nbsp;&nbsp;我觉得你们不会等着三四分钟，于是我设置操作一次后按钮呈禁用状态,两分钟后解禁
      <span slot="footer" class="dialog-footer">
  ...
  methods: {
    ...
    play() {
      ...
      if (this.checkList.length === 2) {
        ...
        let locl = localStorage.getItem("userId");
        this.$axios.post("/likelist?uid=" + locl).then(res => {
